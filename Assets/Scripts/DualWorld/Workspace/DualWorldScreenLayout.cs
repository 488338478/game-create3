...
namespace GameCreate3.DualWorld
{
    ...
    /// <summary>
    /// 双世界两屏布局：左半屏放横板世界（dream / 梦境侧），右半屏放 UI 拼图（reality / 现实侧）。
    /// 通过设置 worldCamera.rect 让相机只在左半渲染；通过设置 realityPanel 锚点让 UI 容器贴右半。
    /// </summary>
        ...
        public void Apply(DualWorldScreenMode mode)
        {
            if (worldCamera != null)
            {
                ...
                worldCamera.rect = mode == DualWorldScreenMode.RealityOnly
                    ...
                    : new Rect(0f, 0f, 0.5f, 1f); // LEFT half = dream/world
            }
            ...
            // RIGHT half = reality / UI puzzle，零内边距：避免 viewport 不覆盖区域露出 GBuffer 残留。
            Stretch(realityPanel, new Vector2(0.5f, 0f), Vector2.one, Vector2.zero, Vector2.zero);
        }
